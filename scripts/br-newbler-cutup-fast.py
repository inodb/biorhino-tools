...
overlapping ones and run Newbler on those. Something similar is apparently done
in [1].
...
Usage:
newbler-cutup-fast <contigs.fast(q|a)> [contigs.fast(q|a) ...]
Options:
...
def cut_up_fast(fastfiles, chunk_size, overlap):
    for ff in fastfiles:
        for record in screed.open(ff):
            if (len(record.sequence) > chunk_size):
                i = 0
                # Output first part of contig twice
                print ">%s.%i\n%s" % (record.name, i,
                        record.sequence[0:chunk_size])
                for split_seq in chunks(record.sequence, chunk_size, overlap):
                    print ">%s.%i\n%s" % (record.name, i, split_seq)
                    i = i + 1
                # Output last chunk twice    
                print ">%s.%i\n%s" % (record.name, i,
                        record.sequence[-chunk_size + overlap:])
            else:
...
def main(argv=None):
    ...
    # process arguments
    if (len(args) >= 1):
        return cut_up_fast(args, chunk_size, overlap)
    else:
